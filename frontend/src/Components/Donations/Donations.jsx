...
export default function Donations() {
  ...
  let [selectedMemberAddress, setSelectedMemberAddress] = useState("");
  ...
  let [selectedLpNumber, setSelectedLpNumber] = useState("None");
  let [selectedMemberName, setSelectedMemberName] = useState("");
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          <label className={styles.label}>
            <select
              ...
              onChange={(event) => {
                ...
                setSelectedMemberName(values[0]);
                let clickId = values[1];
                const selectedMember = membersData.find(
                  (member) => member.id === clickId
                );
                console.log(event.target.value);
                if (selectedMember) {
                  setSelectedMemberAddress(selectedMember.address);
                  setSelectedCenter(selectedMember.center);
                  setSelectedId(selectedMember.id);
                  setSelectedLpNumber(selectedMember.lpNumber);
                } else {
                  setSelectedMemberAddress("");
                  setSelectedCenter(assignedCenter);
                  setSelectedId(""); // Clear the selected member's ID
                }
              }}
              renderValue={(selected) => {
                ...
                if (selectedMember) {
                  return `${selectedMember.name} (ID: ${selectedMember.id})`;
                }
                return selected;
              }}
            >
              ...
              {membersData.map((el, i) => {
                return (
                  <option key={i} value={el.name + ": " + el.id}>
                    {`${el.name} (ID: ${el.id})`}
                  </option>
                );
              })}
            </select>
            <span className={styles.span}>Select Member</span>
          </label>
          <label className={styles.label}>
            <input
              required=""
          ...
          <div className={styles.flex}>
            <label className={styles.label}>
              <input
                ...
                type="text"
                className={styles.input}
                value={selectedLpNumber}
                onChange={(event) => setSelectedLpNumber(event.target.value)}
              />
          ...
          <label className={styles.label}>
            <input
              ...
              className={styles.input}
            />
            <span className={styles.span}>Donation</span>
          </label>
          <label className={styles.label}>
            <input

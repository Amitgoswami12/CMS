...
//     res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' https://cmsicfdr.org; style-src 'self' https://cmsicfdr.org;");
//     next();
//   });
//   app.use((req, res, next) => {
//     if (!req.secure) {
...
} = require("./Features/birthdayFunctions.js");
...
NoAuthRoute(app, "configuration", ConfigurationModel);
...
cron.schedule("0 0 * * *", async () => {
  ...
  try {
    ...
    holidays = holidays.filter((el, i) => {
      return el.start.date == today;
    });
    // Send birthday emails
    Members.forEach(async (member) => {
      for (let i = 0; i < holidays.length; i++) {
        ...
        axios
          .post(
            `https://voice.roundsms.co/api/sendmsg.php?user=ICFDRWA&pass=123456&sender=BUZWAP&phone=${member.phone}&text=jayous&priority=wa&stype=normal&Params=${member.details.name},${holidays[i]},ICFDR,Lord Krishna,${holidays[i]},${holidays[i]},ICFDR Family`
          )
